...
public class FilterUtils {
	...
	public static String getObjectTypeUUID(Database database, Base base) {
		...
		if(base instanceof Tavoite) {
			...
			String uuid = goalTypeProperty.getPropertyValue(level);
			if(uuid == null) return ObjectType.find(database, ObjectType.STRATEGINEN_TAVOITE).uuid;
			return uuid;

		} else if (base instanceof Painopiste) {

			Property focusTypeProperty = Property.find(database, Property.FOCUS_TYPE);
			String uuid = focusTypeProperty.getPropertyValue(level);
			if(uuid == null) return ObjectType.find(database, ObjectType.PAINOPISTE).uuid;
			return uuid;
