...
public class ResponsibilityModelsDialog {
	...
	ResponsibilityModelsDialog(final Main main, Base mapType) {
		...
		nameListener = new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				if(!isListening) return;
				currentSelection.modifyText(main, nameField.getValue());
		...
		final Button add = new Button("Lis‰‰ malli");
		final Button edit = new Button("Muokkaa kentti‰");

		final Button save = new Button("Valitse malli");
		...
		edit.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(edit.getCaption().equals("Muokkaa kentti‰")) {
					...
					edit.setCaption("Tallenna kent‰t");
		...
		add.addClickListener(new ClickListener() {
			...
			@Override
			public void buttonClick(ClickEvent event) {
				new ResponsibilityModel(main.getDatabase(), "Uusi vastuumalli");
				Updates.update(main, true);
				refresh();
			}
		});

		final Window dialog = Dialogs.makeDialog(main, "450px", "650px", "Vastuumallin muokkaus", null, content, buttons2);
